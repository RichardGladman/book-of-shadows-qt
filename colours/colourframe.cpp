...
ColourFrame::ColourFrame(QWidget *parent) :
	QFrame(parent), ui(new Ui::ColourFrame) {
	ui->setupUi(this);

	loadData();
}

ColourFrame::~ColourFrame() { delete ui; }
...
void ColourFrame::on_editButton_clicked() {
	QModelIndexList selectedRows =
		ui->colourTable->selectionModel()->selectedIndexes();
	if (selectedRows.empty()) {
		return;
	}

	ColourForm *form =
		new ColourForm(this, ui->colourTable->model()
...
void ColourFrame::on_deleteButton_clicked() {
	...
	ColourModel model =
		ColourModel::load(ui->colourTable->model()
							  ->index(selectedRows.at(0).row(), 0)
							  .data()
							  .toInt());
