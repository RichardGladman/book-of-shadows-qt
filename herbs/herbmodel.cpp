...
QSqlQuery HerbModel::list(QString search_for)
{
    QSqlQuery query;

    QString select_clause = "SELECT id, name FROM herbs ";
...
QList<HerbModel> HerbModel::load(QString &arcana, int id)
{
    ...
    if (arcana == "runestone") {
        ...
        sql = "SELECT t2.* FROM crystal_herb t1 INNER JOIN herbs t2 ON t1.herb_id = t2.id WHERE t1.crystal_id = ?";
    }
